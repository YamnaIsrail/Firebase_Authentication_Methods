...
class SignInWithGooglenew extends StatefulWidget {
  const SignInWithGooglenew({super.key});

  @override
  State<SignInWithGooglenew> createState() => _SignInWithGooglenewState();
}

class _SignInWithGooglenewState extends State<SignInWithGooglenew> {
  var user_email;
  var user_name;
                  ...
                  children: [
                    InkWell(
                      onTap: () async {
                        ...
                        // Fetch user details from userCredential
                        if (userCredential != null) {
                          setState(() {
                            user_email.text = userCredential.user?.email;
                            user_name.text = userCredential.user!.displayName;
                            userImage = userCredential.user!.photoURL!;
                          });
                          print(" User EEEEEEEEMail${user_email.text}");
                          _showUserInfoDialog(
                            context,
                            user_email.text,
                            user_name.text,
                          ...
                          Navigator.push(
                              context,
                              MaterialPageRoute(
                                  builder: (builder) => ScreenDashboard())
                          );
                        }},
...
void _showUserInfoDialog(BuildContext context, String email, String name, String image) {
  showDialog(
    context: context,
      ...
      return AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('Email: $email'),
            Text('Name: $name'),
            Image.network(image), // Display user image
          ],
        ),
        actions: <Widget>[
          TextButton(
...
class AuthServices{
  final GoogleSignIn _googleSignIn = GoogleSignIn(
    clientId: '450704756196-hq4h06qk08fdbju7nndiffnfmn43vkdv.apps.googleusercontent.com',
  );
  signInWithGoogle() async {
    try {
      final GoogleSignInAccount? googleUser = await _googleSignIn.signIn();
      if (googleUser != null) {
        // You are signed in using Google.
        // You can get user details using googleUser variable.
